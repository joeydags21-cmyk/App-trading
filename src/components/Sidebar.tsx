...
export default function Sidebar() {
  ...
  return (
    <aside className="w-56 h-screen bg-zinc-950 border-r border-zinc-800/60 flex flex-col fixed left-0 top-0">
      {/* Logo */}
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-start gap-2.5 px-3 py-2.5 rounded-lg transition-all duration-100 ${
                active
                  ? 'bg-zinc-800 text-zinc-100'
                  : 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/50'
              }`}
            >
              <span className={`mt-0.5 flex-shrink-0 ${active ? 'text-zinc-300' : 'text-zinc-600'}`}>
                {item.icon}
              </span>
              <div>
      ...
      <div className="px-3 py-4 border-t border-zinc-800/60">
        <button
          ...
          className="flex items-center gap-2.5 w-full px-3 py-2 rounded-lg text-sm text-zinc-600 hover:text-zinc-400 hover:bg-zinc-800/50 transition-all duration-100"
        >
